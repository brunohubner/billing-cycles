...
export default function createBillingCyclesService() {
    ...
    async function update(values) {
        let data = null
        await api
            .put(`/billingCycles/${values.id}`, values)
            .then(resp => (data = resp.data))
            .catch(err => (data = err.response.data))
    ...
    async function remove(values) {
        let data = null
        await api
            .delete(`/billingCycles/${values.id}`)
            .then(resp => (data = resp.data))
            .catch(err => (data = err.response.data))
        return data
    }
    ...
    async function getCount() {
        ...
        await api
            .get("billingCycles/count")
            .then(resp => (count = resp.data || INITIAL_COUNT_STATE))
            .catch(_ => {
                toast.error(
                    "Não foi possível obter a quantidade de registros!",
                    {
                        ...toastConfig
                    }
                )
